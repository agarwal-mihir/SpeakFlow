...
if [[ -z "$APP_BUNDLE" ]]; then
  ...
  elif [[ -d "$WORKDIR/dist-swift/SpeakFlow.app" ]]; then
    APP_BUNDLE="$WORKDIR/dist-swift/SpeakFlow.app"
  fi
fi
...
if [[ -z "$APP_BUNDLE" ]]; then
  echo "SpeakFlow.app not found in /Applications, ~/Applications, or dist-swift/." >&2
  echo "Install the app first or pass APP_BUNDLE=/absolute/path/to/SpeakFlow.app" >&2
  exit 1
fi
